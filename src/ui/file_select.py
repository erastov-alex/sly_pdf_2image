...
def convert_upload(file: str, pdf_path: str, dataset: DatasetApi):
    ...
    with output_progress2(total=pdf_document.page_count) as pbar2:
        for page_number in range(pdf_document.page_count):
            ...
            try:
                info = g.api.image.upload_path(
                    dataset_id=dataset.id, name=image_name, path=image_path
                )
        ...
        try:
            ...
            for object in objects:
                if os.path.isdir(os.path.join("input", object)):
                    ...
                    dataset_folder = g.api.dataset.create(
                        project.id, object, change_name_if_conflict=True
                    )
                    for rootdir, dirs, files in os.walk(path_folder):
                        for file in files:
                            if file.endswith("pdf"):
                                convert_upload(
                                    file,
                                    pdf_path=os.path.join(rootdir, file),
                                    dataset=dataset_folder,
                                )
                                pbar.update(1)
                elif object.endswith("pdf"):
                    convert_upload(
                        object, pdf_path=os.path.join("input", object), dataset=dataset_ground
                    )
                    pbar.update(1)
            output_progress2.hide()
